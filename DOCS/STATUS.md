...
## Current Task
- id: P6-scripted-smoke-five-apis
- title: Добавить **один** скрипт **`scripts/smoke-api-five-entities.mjs`** (или рядом по имени): при **`DATABASE_URL`** + запущенном **`npm run dev`** — последовательные **GET/POST** (или идемпотентные проверки после seed) по всем пяти **`app/api/*`** маршрутам канона; **exit 0** только если все ответы ожидаемы; зафиксировать запуск + результат в **`DOCS/STATUS.md`** (evidence) — **без** U/D, **без** UI, **без** правок **`README`** на этом шаге (если не потребуется одна строка команды)
- status: planned
- linked_phase: 6
...
| «CRUD» для пяти сущностей | **Только `GET` + `POST`** на каждом из **`/api/projects`**, **`/api/outcomes`**, **`/api/assumptions`**, **`/api/acceptance-criteria`**, **`/api/evidence-items`** — **нет** `PUT`/`PATCH`/`DELETE` | **E2E доказан** сейчас только для **`evidence_item`** (POST→GET, **STATUS**); остальные четыре — **реализованы**, но **не** имеют такого же записанного smoke в **`STATUS`** |
| Literal **Update / Delete** | **Отсутствует** для всех пяти | Вне текущего baseline; закрытие фазы 6 по литеральному «CRUD» — **нет** |

**Итог честности:** подфаза «минимальная персистенция + create/read API для пяти таблиц» — **implemented**; **полная** фраза Definition of done **`MASTER` §6** с литеральным **CRUD** — **not_release_ready** / **not** `done` для фазы 6.
...
- [ ] **Симметричный automated smoke** для всех пяти API (не только `evidence_item`) — см. **`Current Task`**
...
## What Is In Progress
- item: **Фаза 6** — сверка **`P6-master-section6-dod-reconcile`** зафиксирована (таблица выше). Следующий шаг реализации — **`P6-scripted-smoke-five-apis`** (см. **`Current Task`** / **`NEXT_STEP.md`**). Фаза **не** закрыта как **`done`** из‑за литерального **U/D** и неполного smoke по четырём API.
...
- **`MASTER` §6 phrase «CRUD … works»:** в узком baseline репозитория сейчас есть **create + read** через API для пяти сущностей; **update/delete** отсутствуют — см. таблицу **Phase 6 — MASTER §6 vs baseline**; литеральное выполнение DoD — **нет**.
